...
create policy "Cheatsheets: student can read own"
  on public.student_cheatsheets
  for select
  using (auth.uid() = student_id);

-- Policies: teacher can manage own
create policy "Cheatsheets: teacher manage own"
  on public.student_cheatsheets
  for all
  using (auth.uid() = teacher_id)
  with check (auth.uid() = teacher_id);

-- Ensure teacher role on insert
create policy "Cheatsheets: only teachers can insert"
  on public.student_cheatsheets
  for insert
  with check (
...
create policy "Learning resources: read assigned"
  on storage.objects
  for select
  using (
...
create policy "Learning resources: teachers can upload"
  on storage.objects
  for insert
  with check (
    bucket_id = 'learning_resources' and exists (
...
create policy "Learning resources: teachers can update"
  on storage.objects
  for update
  using (
    bucket_id = 'learning_resources' and exists (
...
create policy "Learning resources: teachers can delete"
  on storage.objects
  for delete
  using (
    bucket_id = 'learning_resources' and exists (
