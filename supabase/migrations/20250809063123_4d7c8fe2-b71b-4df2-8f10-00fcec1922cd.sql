...
CREATE POLICY IF NOT EXISTS "Homeworks: students can read assigned"
ON public.homeworks
FOR SELECT
USING (
  EXISTS (
...
CREATE POLICY IF NOT EXISTS "Homework files: teacher can upload to own folder"
ON storage.objects
FOR INSERT
TO authenticated
WITH CHECK (
...
CREATE POLICY IF NOT EXISTS "Homework files: teacher can read own"
ON storage.objects
FOR SELECT
TO authenticated
USING (
...
CREATE POLICY IF NOT EXISTS "Homework files: students can read assigned"
ON storage.objects
FOR SELECT
TO authenticated
USING (
